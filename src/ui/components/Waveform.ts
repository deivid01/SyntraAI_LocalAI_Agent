...
export class Waveform {
  ...
  private drawBars(): void {
    ...
    // Calculate average amplitude to send to main process for hotword/clap detection
    let sum = 0;
    for (let i = 0; i < this.dataArray.length; i++) {
      sum += this.dataArray[i];
    }
    const avgAmplitude = (sum / this.dataArray.length) / 255.0;

    if (window.syntra && typeof window.syntra.sendAudioAmplitude === 'function') {
      window.syntra.sendAudioAmplitude(avgAmplitude);
    }
...
(window as any).Waveform = Waveform;
