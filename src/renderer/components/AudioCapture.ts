...
async function startRecording(): Promise<void> {
  try {
    ...
    mediaRecorder.onstop = async () => {
      ...
      if (window.jarvis && typeof window.jarvis.sendRecordedAudio === 'function') {
        window.jarvis.sendRecordedAudio(uint8);
      } else {
...
function setupAudioCaptureIPC(): void {
  ...
  window.jarvis.onStartRecording(() => {
